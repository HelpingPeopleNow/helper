...
- **~165 pytest test functions, CI-gated** — CI (`docker.yml`) runs `pytest` before Docker build on every PR; the `test` job gates `validate` and `push`. Overall coverage **77%**; domain core at 97%, LLM adapters at 100%, metrics at 95%, embedding provider at 92%. Also runs `vector-parity.yml` (byte-parity gate against backend `cmd/hash_fixture`). No lint, no typecheck.
...
- **Ollama adapter uses raw `requests`** — no langchain; full prompt (system+history+user) is concatenated into a single string and POSTed to `/api/generate` with `stream=False, think=False`. Default model `qwen2.5:1.5b` (`OllamaLLMAdapter.__init__` default + `infra/docker-compose.yml` `OLLAMA_MODEL`); env `OLLAMA_MODEL` overrides at runtime.
