...
- **Fallback chain**: Mistral → OpenCode 0 → OpenCode 1 → OpenCode 2 → Ollama (when no explicit provider is set). `HelperAgent.FALLBACK_CHAIN = ["mistral", "opencode0", "opencode1", "opencode2", "ollama"]`; the `mistral` entry is silently skipped at runtime if the adapter wasn't loaded (no `MISTRAL_API_KEY`).
